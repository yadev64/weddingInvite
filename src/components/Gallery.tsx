...
function ParallaxImage({ src, index }: { src: string, index: number }) {
  ...
  const aspectClass = index % 3 === 0 ? "aspect-[3/4]" : index % 3 === 1 ? "aspect-[4/5]" : "aspect-[1/1]";

  return (
    <div ref={ref} className={`relative w-full ${aspectClass}`}>
      <motion.div 
        style={{ y }}
        className="relative w-full h-full rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(212,175,55,0.15)] border border-primary/20 group cursor-pointer"
      >
        <img 
          src={src} 
          alt="Gallery moment" 
    ...
    <section className="py-32 px-4 md:px-8 bg-background relative overflow-hidden z-10">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="font-great-vibes text-6xl md:text-8xl mb-4 drop-shadow-[0_0_20px_rgba(212,175,55,0.3)] text-gold-gradient">
            Captured Moments
          </h2>
        </motion.div>
